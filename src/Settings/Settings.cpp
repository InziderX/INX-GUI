...
//
// INX is free software: you can redistribute it and/or modify
...
const char OPTION_PRIVACY_NEWS_ENABLED[] = "newsDisabled";

const char DEFAULT_WALLET_FILE_NAME[] = "inx.wallet";
...
const char* VERSION_NAME = "Sophia";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.inziderx.io:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("inxwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("inxwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("inxwallet.desktop");
  ...
  res = autorunSettings.contains("INXWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("INXWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.inx.inxwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=INX Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("INXWallet", appPath);
    } else {
      autorunSettings.remove("INXWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\inx", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:inx");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\inx\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\inx\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("inxwallet.cfg"));
