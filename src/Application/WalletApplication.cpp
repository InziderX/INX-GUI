...
// Copyright (c) 2017-2018, Spes Coin developers
// Copyright (c) 2018-2019, INX developers
//
...
const char INX_URI_SCHEME_NAME[] = "cryptoinx";
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("inxwallet.lock"));
  QUrl paymentUrl = QUrl::fromUserInput(arguments().last());
  if (paymentUrl.scheme() != INX_URI_SCHEME_NAME) {
    paymentUrl = QUrl();
  }
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] INX wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "INX wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    if (m_splash != nullptr) {
      m_splash->show();
      m_splash->showMessage(QObject::tr("Loading INX blockchain..."), Qt::AlignLeft | Qt::AlignBottom, Qt::white);
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have INXd with non-default RPC port, you should terminate it and relaunch INX Wallet\n"
      "or\n"
      "Set the Local daemon required port in INX Wallet Menu/Preferences/Connection settings."));
	  if (m_splash != nullptr)
		  m_splash->hide();
      dlg.exec();
      return false;
